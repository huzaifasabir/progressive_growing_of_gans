...
def setup_snapshot_image_grid(G, training_set,
    ...
    multilabels = np.zeros([gw * gh, training_set.multilabel_size], dtype=training_set.multilabel_dtype)
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label, embedding, multilabel = training_set.get_minibatch_np(1)
            ...
            multilabels[idx] = multilabel[0]
            break

    # Generate latents.
    latents = misc.random_latents(gw * gh, G)
    return (gw, gh), reals, labels, embeddings, multilabels, latents
...
def train_progressive_gan(
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G = tfutil.Network('G', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, embedding_size=training_set.embedding_size, multilabel_size=training_set.multilabel_size, **config.G)
            D = tfutil.Network('D', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, embedding_size=training_set.embedding_size, multilabel_size=training_set.multilabel_size, **config.D)
            Gs = G.clone('Gs')
    ...
    with tf.name_scope('Inputs'):
        ...
        reals, labels, embeddings, multilabel   = training_set.get_minibatch_tf()
        #print(tfutil.run(labels))
        reals_split     = tf.split(reals, config.num_gpus)
        labels_split    = tf.split(labels, config.num_gpus)
        embeddings_split    = tf.split(embeddings, config.num_gpus)
        multilabel_split = tf.split(multilabel, config.num_gpus)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            multilabel_gpu = multilabel_split[gpu]
            ...
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, embeddings=embeddings_gpu, multilabel=multilabel_gpu, **config.D_loss)
    ...
    grid_size, grid_reals, grid_labels, grid_embeddings, grid_multilabel, grid_latents = setup_snapshot_image_grid(G, training_set, **config.grid)
    sched = TrainingSchedule(total_kimg * 1000, training_set, **config.sched)
    grid_fakes = Gs.run(grid_latents, grid_labels, grid_embeddings, grid_multilabel, minibatch_size=sched.minibatch//config.num_gpus)
