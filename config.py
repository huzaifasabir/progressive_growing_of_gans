...
train = EasyDict(func='util_scripts.discriminator_evaluation', run_id='038-pgan-tf50ksorted-preset-v2-2gpus-fp32'); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
#train = EasyDict(func='util_scripts.generate_fake_images', run_id='038-pgan-tf50ksorted-preset-v2-2gpus-fp32', num_pngs=32); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id='046-pgan-tf50ksorted-preset-v2-4gpus-fp32', log='metric-msssim-20k.txt', metrics=['msssim'], num_images=20000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
