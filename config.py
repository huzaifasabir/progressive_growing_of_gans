...
#tf_config = tf_config.ConfigProto()
#tf_config.gpu_options.per_process_gpu_memory_fraction = 0.9
#env = tf.Session(config=tf_config, ...)
...
desc += '-tf50_ref_chris';        dataset = EasyDict(tfrecord_dir='without_refrence_chiristian');        train.mirror_augment = True
...
#train = EasyDict(func='util_scripts.generate_fake_images', run_id='066-pgan-tf50_ref_chris-preset-v2-1gpu-fp32', num_pngs=500); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=23, log='metric-fid-50k.txt', metrics=['fisd'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
...
train = EasyDict(func='util_scripts.evaluate_metrics', run_id='066-pgan-tf50_ref_chris-preset-v2-1gpu-fp32', log='metric-msssim-50k.txt', metrics=['msssim'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
